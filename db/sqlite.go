...
// 插入数据
func insertSQLite(table string, data []string) (err error) {
	// 获取数据库连接句柄
	dbHandle, err := connectSQLite()
	...
	// 检查数据库结构
	err = checkDBStructureSQLite(dbHandle)
	if err != nil {
...
// - 支持AND，不支持OR
// - 只支持=，不支持其他类别
// 如果要使用更高级的操作，建议执行execSQLite方法
func selectSQLite(table string, query map[string]string) (queryResults map[int]map[string]string, err error) {
	// 获取数据库连接句柄
	dbHandle, err := connectSQLite()
	defer dbHandle.Close()
	if err != nil {
...
// - 只能够对数值进行自增处理
// 如果要使用更高级的操作，建议执行execSQLite方法
func updateSQLite(table string, updateItem string, updateValue int, query map[string]string) (err error) {
	// 获取数据库连接句柄
	dbHandle, err := connectSQLite()
	defer dbHandle.Close()
	if err != nil {
...
// 执行任意SQL语句
// 部分代码类似于selectSQLite
func execSQLite(query string) (execResults map[int]map[string]string, err error) {
	// 获取数据库连接句柄
	dbHandle, err := connectSQLite()
	defer dbHandle.Close()
	if err != nil {
...
// 连接数据库
func connectSQLite() (db *sql.DB, err error) {
	SQLiteConfig, err := getConfigSQLite()
	if err != nil {
		return nil, errors.New("数据库连接失败，原因：" + err.Error())
	}

	// 启动连接
	db, err = sql.Open("sqlite3", SQLiteConfig)
	if err != nil {
		return nil, errors.New("数据库连接失败，原因：" + err.Error())
	}
...
// 读取数据库配置
func getConfigSQLite() (config string, err error) {
	// 读取配置文件
